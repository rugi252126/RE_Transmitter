...
void LcdDisplay::lcdF_Init(void)
{
    ...
    copy_browse_menu_e_ = MENU_DISPLAY_MAX_E;
    diplay_refresh_time_ = 0u;
}

// ---------------------------------------------------------------------------
// Cyclic function
void LcdDisplay::lcdF_Cyclic(void)
{
...
void LcdDisplay::lcdLF_generalInfoDisplay(void)
{
    bool                     tmp_b;
    ...
    tmp_b = rte_ivar.Rte_Read_Transceiver_Status();
    if(tmp_b != false)
    {
    ...
    tmp_mode_e = rte_ivar.Rte_Read_JoyStick_ControllerMode();
...
void LcdDisplay::lcdLF_voltageInfoDisplay(void)
{
    enum voltage_status_et   tmp_volt_stat_e;
    float                    tmp_volt_disp_f;

    tmp_volt_disp_f = rte_ivar.Rte_Read_VoltageMonitoring_ActualVoltage();
    lcd_display.setCursor(0,0);
    ...
    tmp_volt_stat_e = rte_ivar.Rte_Read_VoltageMonitoring_VoltageStatus();
...
void LcdDisplay::lcdLF_joyStickInfoDisplay(void)
{
    uint8_t              tmp_u8;
    enum direction_et    tmp_dir_e;

    // Throttle forward and backward
    tmp_u8 = rte_ivar.Rte_Read_JoyStick_ThrottleFwdBckwrd();
    lcd_display.setCursor(0,0);
    ...
    tmp_u8 = rte_ivar.Rte_Read_JoyStick_ThrottleLeftRight();
    ...
    tmp_dir_e = rte_ivar.Rte_Read_JoyStick_ControlFwdBckwrd();
    lcd_display.setCursor(0,2);
    ...
    tmp_dir_e = rte_ivar.Rte_Read_JoyStick_ControlLeftRight();
...
// ---------------------------------------------------------------------------
// Browse menu - JoyStick information display
void LcdDisplay::lcdLF_switchInfoDisplay(void)
{
    bool tmp_b;

    // Switch 1
    tmp_b = rte_ivar.Rte_Read_SwitchIn_Switch1();
    lcd_display.setCursor(0,0);
    ...
    // Switch 2
    tmp_b = rte_ivar.Rte_Read_SwitchIn_Switch2();
    lcd_display.setCursor(0,1);
    ...
    // Switch 3
    tmp_b = rte_ivar.Rte_Read_SwitchIn_Switch3();
    lcd_display.setCursor(0,2);
    ...
    // Switch 4
    tmp_b = rte_ivar.Rte_Read_SwitchIn_Switch4();
    lcd_display.setCursor(0,3);
...
void LcdDisplay::lcdLF_potiInfoDisplay(void)
{
    uint8_t tmp_u8;

    tmp_u8 = rte_ivar.Rte_Read_Potentiometer_Poti1Data();
    lcd_display.setCursor(0,0);
...
void LcdDisplay::lcdLF_logisticDataInfoDisplay(void)
{
    // SW version
    lcd_display.setCursor(0,0);
...
void LcdDisplay::lcdLF_controlledObjectInfoDisplay(void)
{
    uint8_t tmp_u8=0u;
...
void LcdDisplay::lcdLF_browseMenuDisplay(void)
{      
    enum browse_menu_et      tmp_browse_menu_e;


    tmp_browse_menu_e = rte_ivar.Rte_Read_Menu_BrowseMenu(); 
    if(tmp_browse_menu_e != copy_browse_menu_e_)
    {
        // clear all displays if new browse state has changed.
        lcd_display.clear();
        diplay_refresh_time_ = LCD_DISPLAY_REFRESH_TIME_K; // there is new state. immediately display the info without refresh time
        copy_browse_menu_e_ = tmp_browse_menu_e;
    }
    else
    {
        // no action
    ...
    if(diplay_refresh_time_ >= LCD_DISPLAY_REFRESH_TIME_K)
    {
        diplay_refresh_time_ = 0u; // clear refresh time
    ...
    else
    {
        ...
        diplay_refresh_time_ += 1;
    }
}

// ---------------------------------------------------------------------------
// Set of main menu displays
//void LcdDisplay::lcdLF_mainMenuDisplay(void)
//{
    //lcd_display.setCursor(0,0);
